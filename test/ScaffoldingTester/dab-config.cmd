...
dab add "Alphabeticallistofproducts" --source "[dbo].[Alphabetical list of products]" --fields.include "ProductID,ProductName,SupplierID,CategoryID,QuantityPerUnit,UnitPrice,UnitsInStock,UnitsOnOrder,ReorderLevel,Discontinued,CategoryName" --source.key-fields "ProductID" --permissions "anonymous:*" 
@echo No primary key found for table/view Category Sales for 1997, using first column (CategoryName) as key field
dab add "CategorySalesfor1997" --source "[dbo].[Category Sales for 1997]" --fields.include "CategoryName,CategorySales" --source.key-fields "CategoryName" --permissions "anonymous:*" 
@echo No primary key found for table/view Current Product List, using first Id column (ProductID) as key field
dab add "CurrentProductList" --source "[dbo].[Current Product List]" --fields.include "ProductID,ProductName" --source.key-fields "ProductID" --permissions "anonymous:*" 
@echo No primary key found for table/view Customer and Suppliers by City, using first column (City) as key field
dab add "CustomerandSuppliersbyCity" --source "[dbo].[Customer and Suppliers by City]" --fields.include "City,CompanyName,ContactName,Relationship" --source.key-fields "City" --permissions "anonymous:*" 
@echo No primary key found for table/view Invoices, using first Id column (CustomerID) as key field
dab add "Invoices" --source "[dbo].[Invoices]" --fields.include "ShipName,ShipAddress,ShipCity,ShipRegion,ShipPostalCode,ShipCountry,CustomerID,CustomerName,Address,City,Region,PostalCode,Country,Salesperson,OrderID,OrderDate,RequiredDate,ShippedDate,ShipperName,ProductID,ProductName,UnitPrice,Quantity,Discount,ExtendedPrice,Freight" --source.key-fields "CustomerID" --permissions "anonymous:*" 
@echo No primary key found for table/view Order Details Extended, using first Id column (OrderID) as key field
dab add "OrderDetailsExtended" --source "[dbo].[Order Details Extended]" --fields.include "OrderID,ProductID,ProductName,UnitPrice,Quantity,Discount,ExtendedPrice" --source.key-fields "OrderID" --permissions "anonymous:*" 
@echo No primary key found for table/view Order Subtotals, using first Id column (OrderID) as key field
dab add "OrderSubtotals" --source "[dbo].[Order Subtotals]" --fields.include "OrderID,Subtotal" --source.key-fields "OrderID" --permissions "anonymous:*" 
@echo No primary key found for table/view Orders Qry, using first Id column (OrderID) as key field
dab add "OrdersQry" --source "[dbo].[Orders Qry]" --fields.include "OrderID,CustomerID,EmployeeID,OrderDate,RequiredDate,ShippedDate,ShipVia,Freight,ShipName,ShipAddress,ShipCity,ShipRegion,ShipPostalCode,ShipCountry,CompanyName,Address,City,Region,PostalCode,Country" --source.key-fields "OrderID" --permissions "anonymous:*" 
@echo No primary key found for table/view Product Sales for 1997, using first column (CategoryName) as key field
dab add "ProductSalesfor1997" --source "[dbo].[Product Sales for 1997]" --fields.include "CategoryName,ProductName,ProductSales" --source.key-fields "CategoryName" --permissions "anonymous:*" 
@echo No primary key found for table/view Products Above Average Price, using first column (ProductName) as key field
dab add "ProductsAboveAveragePrice" --source "[dbo].[Products Above Average Price]" --fields.include "ProductName,UnitPrice" --source.key-fields "ProductName" --permissions "anonymous:*" 
@echo No primary key found for table/view Products by Category, using first column (CategoryName) as key field
dab add "ProductsbyCategory" --source "[dbo].[Products by Category]" --fields.include "CategoryName,ProductName,QuantityPerUnit,UnitsInStock,Discontinued" --source.key-fields "CategoryName" --permissions "anonymous:*" 
@echo No primary key found for table/view Quarterly Orders, using first Id column (CustomerID) as key field
dab add "QuarterlyOrders" --source "[dbo].[Quarterly Orders]" --fields.include "CustomerID,CompanyName,City,Country" --source.key-fields "CustomerID" --permissions "anonymous:*" 
@echo No primary key found for table/view Sales by Category, using first Id column (CategoryID) as key field
dab add "SalesbyCategory" --source "[dbo].[Sales by Category]" --fields.include "CategoryID,CategoryName,ProductName,ProductSales" --source.key-fields "CategoryID" --permissions "anonymous:*" 
@echo No primary key found for table/view Sales Totals by Amount, using first Id column (OrderID) as key field
dab add "SalesTotalsbyAmount" --source "[dbo].[Sales Totals by Amount]" --fields.include "SaleAmount,OrderID,CompanyName,ShippedDate" --source.key-fields "OrderID" --permissions "anonymous:*" 
@echo No primary key found for table/view Summary of Sales by Quarter, using first Id column (OrderID) as key field
dab add "SummaryofSalesbyQuarter" --source "[dbo].[Summary of Sales by Quarter]" --fields.include "ShippedDate,OrderID,Subtotal" --source.key-fields "OrderID" --permissions "anonymous:*" 
@echo No primary key found for table/view Summary of Sales by Year, using first Id column (OrderID) as key field
dab add "SummaryofSalesbyYear" --source "[dbo].[Summary of Sales by Year]" --fields.include "ShippedDate,OrderID,Subtotal" --source.key-fields "OrderID" --permissions "anonymous:*" 
