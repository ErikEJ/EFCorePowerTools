    ...
    public static class CliConfigMapper
    {
            ...
            string renamingPath)
        {
            ...
            var options = new ReverseEngineerCommandOptions
            {
                ...
                IrregularWords = replacements.IrregularWords?.Select(w => (w.Singular, w.Plural, w.MatchEnding)).ToList(),
                UncountableWords = replacements.UncountableWords?.ToList(),
                PluralRules = replacements.PluralWords?.Select(w => (w.Rule, w.Replacement)).ToList(),
                SingularRules = replacements.SingularWords?.Select(w => (w.Rule, w.Replacement)).ToList(),
